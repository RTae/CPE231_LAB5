...
$(document).ready( function () {
    ...
    $('.search_paymentmedthod_code').click(function () {
        console.log('Click paymentMedthod')
        $.ajax({
            url:  '/paymentMedthod/list',
            type:  'get',
            dataType:  'json',
            success: function  (data) {
                let rows =  '';
                var i = 1;
                data.paymentMedthods.forEach(paymentMedthod => {
                    rows += `
                    <tr class="d-flex">
                        <td class='col-1'>${i++}</td>
                        <td class='col-3'><a class='a_click' href='#'>${paymentMedthod.code}</a></td>
                        <td class='col-5'>${paymentMedthod.name}</td>
                        <td class='col-3'></td>
                        <td class='hide'></td>
                    </tr>`;
                });
        ...
        // open popup
        $('#txt_modal_param').val('paymentmedthod_code');
        $('#modal_form').modal();
    });
    ...
    $('body').on('click', 'a.a_click', function() {
        ...
        } else if ($('#txt_modal_param').val() == 'paymentmedthod_code') {
        ...
        success: function  (data) {
            ...
            $("#table_main tbody tr").each(function() {
                if (i < data.receiptlineitem.length) {
                    ...
                    $(this).find('.aph').html(data.receiptlineitem[i].aph);
                }
                i++;
            });
            re_calculate_total_price();
...
function re_calculate_total_price () {
    ...
    $("#table_main tbody tr").each(function() {
        ...
        var APH = $(this).find('.aph').html();
        $(this).find('.aph').html(parseInt(APH));
