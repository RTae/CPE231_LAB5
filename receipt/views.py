...
class PaymentMedthodList(View):
    def get(self, request):
        paymentMedthods = list(PaymentMedthod.objects.all().values())
        data = dict()
        data['paymentMedthods'] = paymentMedthods
        response = JsonResponse(data)
        response["Access-Control-Allow-Origin"] = "*"
        return response
...
class ReceiptDetail(View):
    def get(self, request, pk, pk2):
        ...
        receiptlineitem = list(ReceiptLineItem.objects.select_related('invoice_no').filter(receipt_no=receipt_no).order_by('lineitem').values("lineitem","receipt_no","invoice_no","invoice_no__date","invoice_no__amount_due","aph"))
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for lineitem in dict_lineitem['lineitem']:
                ...
                lineitem['aph'] = reFormatNumber(lineitem['aph'])
...
class ReceiptPDF(View):
    def get(self, request, pk, pk2):
        ...
        for line in receiptlineitem:
            line['iar'] = line['invoice_no__amount_due'] - line['aph']
